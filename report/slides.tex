...
\begin{itemize}
    \item<1-> Pivoted Cholesky: before each Cholesky step, select pivot (largest element of diagonal) and swap row \& column
    \item<2-> The result is lower triangular up to some row permutations:
\end{itemize}
...
\begin{itemize}
    \item<4-> After $k < n$ steps, we obtain a rank $k$ approximation $K_{XX} \approx L_k L_k^\top$
